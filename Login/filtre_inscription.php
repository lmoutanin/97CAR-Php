    ...
    if (!empty($email) && !empty($mdp) && !empty($civilite)  &&  !empty($prenom) &&  !empty($nom)   &&  !empty($adresse)   &&  !empty($codePostal)   &&  !empty($ville) &&  !empty($telephone)) {
        ...
        if (filter_var($email_nettoye, FILTER_VALIDATE_EMAIL)) {
            ...
            if (!$client) {
                ...
                $req = $bdd->prepare("INSERT INTO client (id_client,mel,mdp,civilite,prenom,nom,adresse,code_postal,ville,telephone) VALUES(0,:mel,:mdp,:civilite,:prenom,:nom,:adresse,:code_postal,:ville,:telephone)");
                $req->execute(array('mel' => $email_nettoye, 'mdp' => $mdp_nettoye, 'civilite' => $civilite_nettoye, 'prenom' => $prenom_nettoye, 'nom' => $nom_nettoye, 'adresse' =>  $adresse_nettoye, 'code_postal' => $codePostal_nettoye, 'ville' => $ville_nettoye, 'telephone' => $telephone_nettoye));
                $connexion = true;
